...
def _load_specific_report_context(file_id: str):
    """
    Load report content and assessment specifically for a given file_id.
    Replaces _load_latest_assessment_for_chat and _load_report_content_for_chat
    """
    try:
        backend_dir = Path(__file__).parent.parent.parent
        ...
        if json_file.exists():
            ...
            # Reconstruct objects (Simplified for brevity, assuming models exist)
            from .models import ComplianceAssessment, DisclosureAnalysis, DisclosureStatus
            
            # Rebuild metric analyses list...
            metric_analyses = []
            for item in assessment_data.get("metric_analyses", []):
                # ... (Keep your existing reconstruction logic here) ...
                # For brevity, reusing the logic from your original _load_latest...
            ...
            assessment = ComplianceAssessment(
                report_id=assessment_data.get("report_id", file_id),
                total_metrics_analyzed=assessment_data.get("total_metrics_analyzed", 0),
                overall_compliance_score=assessment_data.get("overall_compliance_score", 0),
        ...
        # We need to find the extracted markdown. Usually named {filename}_extracted.md
        # But we only have file_id. We might need to search or rely on file_manager metadata.
        # Fallback: Search for any markdown containing the file_id or look in outputs
        
        # Try to find the file via file_manager first if possible, or search dir
        markdown_content = None
        report_content_obj = None
        
        # Search strategy: look for files containing the file_id
        # Note: Your upload process creates files like {base_name}_extracted.md
        # You might need to store the mapping of file_id -> extracted_filename explicitly if it's not consistent.
        
        # Assuming you can find the file (reuse your existing loading logic but filter by ID)
        uploads_dir = Path(__file__).parent.parent.parent.parent / "uploads"
        # Try finding a markdown file that matches the file ID part if possible
        # For now, let's assume we can match via the assessment's report_id or similar
        
        # ... (Insert logic to load ReportContent object similar to your original function) ...
        
        return assessment, report_content_obj

    except Exception as e:
        logger.error(f"Error loading context for {file_id}: {e}")
        return None, None
...
async def get_file_chat_history(file_id: str):
    """
    Get persistent chat history for a specific file
    """
...
async def chat_with_file(file_id: str, request: ChatRequest):
...
async def clear_file_chat(file_id: str):
