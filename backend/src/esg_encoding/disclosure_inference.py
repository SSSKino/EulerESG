...
class DisclosureInferenceEngine:
    ...
    def _analyze_single_metric(
        ...
        try:
            ...
            # Defensive against markdown backticks:
            ...
            if (json_match):
                ...
                fallback_match = re.search(r"\{.*\}", response.choices[0].message.content, re.DOTALL)
                if (fallback_match): 
                    json_output = fallback_match.group(0)
                elif (response.choices[0].message.content[0] != "{"):
                    json_output += "{"
                    json_output += json_match
                    # Some model (i.e. GPT-5) auto-completes the JSON
                else:
                    raise ValueError("NO JSON BLOCKS DETECTED FROM LLM RESPONSE! Check above debug response for details")
           
            llm_result = json.loads(json_output)
