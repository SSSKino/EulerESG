...
/** Newest upload first; tie-breaker by name. */
...
const FileTable: React.FC<FileTableProps> = ({ onChatClick, selectedRows, onSelectionChange }) => {
  ...
  // Auto-refresh backend statuses while there are unfinished files.
  // This keeps pending -> ready transitions visible without manual refresh.
  useEffect(() => {
    const timer = setInterval(() => {
      const state = useFileStore.getState();
      const hasUnfinished = state.files.some(
  ...
  const columns: ColumnsType<File> = [
    ...
    },
    {
      ...
      render: (_: any, file: File) => (
        <Tag color={statusColor(file.status)}>{statusText(file)}</Tag>
      ),
    },
    {
        ...
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await useFileStore
                .getState()
                .deleteFile(file.file_id!, file.analysis_scope_key);
            }}
            okText={t("common.yes")}
      ...
      <div className="overflow-x-auto px-1 pb-2">
        {dataSource.length === 0 && !loading ? (
          ...
          <Table
            ...
            size="small"
            className="w-full"
            rowKey={(record) => record.key || record.file_id}
            loading={loading}
            rowSelection={{
              selectedRowKeys: selectedRows.map((r) => r.key || r.file_id),
